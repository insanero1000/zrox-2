    ...
    try {
      ...
      if (time.toLowerCase().includes("pe")) {
        ...
        message.channel.send({
          ...
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:yes:833101995723194437> \`${member.user.tag}\` got **MUTED**`)
        ...
        member.send({
          ...
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:yes:833101995723194437> \`${message.author.tag}\` muted you in ${message.guild.name}`)
          .setDescription(`Reason:\n> ${reason ? `${reason.substr(0, 1800)}` : `NO REASON`}`)
        }).catch((_) => {})
      } else {
        if (!mutetime || mutetime === undefined) {
          return message.channel.send({
            embed: new MessageEmbed()
              .setColor(es.wrongcolor).setFooter(es.footertext, es.footericon)
              .setTitle(`<:no:833101993668771842> please add a TIME!`)
              .setDescription(`Usage: \`${prefix}mute @User <Time+Format(e.g: 10m) / perma> [REASON]\`\n\nExample: \`${prefix}mute @User 10m He is doing bad stuff!\`\n\nExample2: \`${prefix}mute @User perma He is doing bad stuff!\``)
          });
        }
        //Send information in the Chat
        message.channel.send({
        ...
        member.send({
          ...
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:yes:833101995723194437> \`${message.author.tag}\` muted you in ${message.guild.name} for \`${ms(mutetime, { long: true })}\``)
          .setDescription(`Reason:\n> ${reason ? `${reason.substr(0, 1800)}` : `NO REASON`}`)
        }).catch((_) => {})
      }
