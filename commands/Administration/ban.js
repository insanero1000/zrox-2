  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      return message.channel.send(new Discord.MessageEmbed()
        ...
        .setTitle("<:no:833101993668771842> I am missing the permission to `BAN MEMBERS`!")
      )
      ...
      if (moderationPosition <= memberPosition)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:no:833101993668771842> I cannot ban someone, who is above/equal you`)
        );
      ...
      if (!kickmember.bannable)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:no:833101993668771842> The Member is not bannable, sorry!`)
        );
      try{
        kickmember.send(new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:yes:833101995723194437> You got banned by \`${message.author.tag}\` from \`${message.guild.name}\` for ${days === 0 ? `Infinite Days` : `${days} Days`}`)
      ...
      try {
        kickmember.ban({
          ...
          message.channel.send(new MessageEmbed()
            .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
            .setFooter(es.footertext, es.footericon)
            .setTitle(`<a:yes:833101995723194437> Banned \`${kickmember.user.tag}\` for ${days === 0 ? `Infinite Days` : `${days} Days`}`)
            .setDescription(`Reason:\n> ${reason}`)
