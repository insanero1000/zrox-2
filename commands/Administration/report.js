    ...
    try {      
      if(client.settings.get(message.guild.id, `reportlog`) == "no") 
        return message.channel.send(new MessageEmbed()
            ...
            .setTitle(`<:no:833101993668771842> Report System is not setup yet!`)
      ...
      if (member.roles.highest.position > message.member.roles.highest.position)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:no:833101993668771842> I cannot report this Member, because he is higher to your Rang Position!`)
        );
      ...
      if (!reason)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:no:833101993668771842> please add a REASON!`)
          .setDescription(` Usage: \`${prefix}report @User <REASON>\`\n\nExample: \`${prefix}report @User 10m He is doing bad stuff!\``)
        );
      ...
      message.channel.send(new MessageEmbed()
        ...
        .setFooter(es.footertext, es.footericon)
        .setTitle(`<a:yes:833101995723194437> \`${member.user.tag}\` got **REPORTED** for:`)
        .setDescription(`Reason:\n> ${reason ? `${reason.substr(0, 1800)}` : `NO REASON`}`)
      );
      member.send(new MessageEmbed()
        .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
        .setFooter(es.footertext, es.footericon)
        .setTitle(`<a:yes:833101995723194437> \`${message.author.tag}\` reported you for:`)
        .setDescription(`Reason:\n> ${reason ? `${reason.substr(0, 1800)}` : `NO REASON`}`)
      );
